...
import { E1, E2, E3, P1, P2, P3 } from "../cards/Evaluation"
import { useFlipAnimation } from "@/hooks/useFlipAnimation"
import { useAnimationCoordinator } from "@/hooks/useAnimationCoordinator"

gsap.registerPlugin(Flip, ScrollTrigger)

export default function EvaluationFlipAnimation() {
  const [stage, setStage] = React.useState<1 | 2 | 3 | 4 | 5 | 6>(1)
  const [isManual, setIsManual] = React.useState(false)
  const [hasStarted, setHasStarted] = React.useState(false)
  const autoPlayTimerRef = React.useRef<NodeJS.Timeout | null>(null)
  const containerRef = React.useRef<HTMLDivElement>(null)
  const { requestStart, notifyComplete } = useAnimationCoordinator("evaluation")
  ...
  const handleStepForward = React.useCallback(() => {
    ...
    const transitions: Record<number, [HTMLElement | null, HTMLElement | null]> = {
      ...
      5: [card5Ref.current, card6Ref.current],
    }
  ...
  const handleStepBackward = React.useCallback(() => {
    ...
    const transitions: Record<number, [HTMLElement | null, HTMLElement | null]> = {
      ...
      6: [card6Ref.current, card5Ref.current],
    }
  ...
  React.useEffect(() => {
    ...
    if (stage === 1) {
      ...
      autoPlayTimerRef.current = setTimeout(() => {
        animateFlip(card5Ref.current, card6Ref.current, {
          duration: 2.0,
          ease: "power2.inOut",
          onComplete: () => {
            setStage(6)
    ...
    <div ref={containerRef} className="relative">
      ...
      {stage < 3 && (
        <div className="relative">
          ...
          >
            <E1 stageKey={stage === 1 ? "e1-init" : undefined} isActive={!isManual && stage === 1} />
          </div>
          <div
            ref={card2Ref}
            data-flip-id="card-2"
          ...
          >
            <E2 stageKey={stage === 2 ? "e2-init" : undefined} isActive={!isManual && stage === 2} />
          </div>
        </div>
      )}
      ...
      {stage >= 3 && (
        <div className="flex flex-col lg:flex-row items-start gap-[5px]">
          <div
            ...
            data-flip-id="card-2"
          >
            <E2 stageKey="e2-static-completed" isActive={false} />
          </div>
          ...
          <div className="relative flex items-stretch">
            ...
            >
              <E3 stageKey={stage === 3 ? "e3-init" : undefined} isActive={!isManual && stage === 3} />
            </div>
            <div
              ref={card4Ref}
              data-flip-id="card-4"
            ...
            >
              <P1 stageKey={stage === 4 ? "p1-init" : undefined} isActive={!isManual && stage === 4} />
            </div>
          </div>
          ...
          {stage >= 5 && (
            <div className="relative flex items-stretch">
              ...
              >
                <P2 stageKey={stage === 5 ? "p2-init" : undefined} isActive={!isManual && stage === 5} />
              </div>
              <div
                ref={card6Ref}
                data-flip-id="card-6"
              ...
              >
                <P3 stageKey={stage === 6 ? "p3-init" : undefined} isActive={!isManual && stage === 6} />
              </div>
            </div>
          )}
        </div>
